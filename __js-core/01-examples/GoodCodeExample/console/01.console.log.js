/*
** 01.console.log.js
**
*/

const rec1 = {name: 'shalom', age: 30, state:'usa', single: false};
const rec2 = {name: 'yifat', age: 40, state:'il', single: false};
const rec3 = {name: 'yaron', age: 46, state:'il', single: false};

console.log('01.console.log.js');
console.log('using the console to display object "programmer title" and "property value" in the console/terminal');
console.log('variable object name will not be displayed --> rec1, rec2, rec3');

console.log('_name: ' ,rec1.name , ' _age: ' ,rec1.age , ' _state: ' ,rec1.state , ' _single: ' ,rec1.single );
console.log('_name: ' ,rec2.name , ' _age: ' ,rec2.age , ' _state: ' ,rec2.state , ' _single: ' ,rec2.single );
console.log('_name: ' ,rec3.name , ' _age: ' ,rec3.age , ' _state: ' ,rec3.state , ' _single: ' ,rec3.single );
